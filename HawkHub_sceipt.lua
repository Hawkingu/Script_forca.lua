...
ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling  -- Garante que a GUI esteja acima de outros elementos

local LogoButton = Instance.new("TextButton", ScreenGui)
LogoButton.Position = UDim2.new(0, 10, 0, 10)
LogoButton.Size = UDim2.new(0, 50, 0, 50)
LogoButton.Text = "🕴"
LogoButton.Name = "HAWKEXECUT+"
LogoButton.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
LogoButton.TextColor3 = Color3.fromRGB(255, 255, 255)
LogoButton.ZIndex = 2  -- Garantir que a logo apareça acima de outros elementos
...
MainFrame.ZIndex = 1  -- O painel deve estar abaixo da logo
...
for i, name in ipairs(Tabs) do
    ...
    tab.MouseButton1Click:Connect(function()
        for _, fr in pairs(TabScripts) do
            fr.Visible = false
        end
        frame.Visible = true
        ActiveTab = name
...
-- FARM
createToggle("Auto Força (+5)", TabScripts.Farm, function(state)
    _G.autoForca = state
    spawn(function()
...
createToggle("Auto Vida (+1)", TabScripts.Farm, function(state)
    _G.autoVida = state
    spawn(function()
...
createToggle("Auto Stamina", TabScripts.Farm, function(state)
    if state then
...
-- BRINCADEIRA
createToggle("Atravessar Parede", TabScripts.Brincadeira, function(state)
    _G.noclip = state
    spawn(function()
        while _G.noclip do
            wait()
...
createToggle("Voar no Chão", TabScripts.Brincadeira, function(state)
    _G.flyMove = state
    local flying = false
    local speed = 50
    ...
    RS:BindToRenderStep("HawkFly", Enum.RenderPriority.Character.Value, function()
        if _G.flyMove then
            flying = true
            local moveDir = UIS:GetFocusedTextBox() == nil and LocalPlayer:GetMouse().Hit.lookVector or Vector3.new()
...
createToggle("Pegar Fogo", TabScripts.Brincadeira, function(state)
    if state then
        local char = LocalPlayer.Character
