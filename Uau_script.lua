		...
		while ativo do
			for _, obj in pairs(workspace:GetDescendants()) do
				if obj:IsA("BasePart") and obj.Name == nomeSaco and estaPerto(obj) then
					-- Aqui o código para bater no saco, vamos tentar acessar uma animação de soco ou um método
					local humanoide = char:FindFirstChild("Humanoid")
					if humanoide then
						-- Aqui você pode substituir por uma ação de ataque ou uma animação específica
						humanoide:MoveTo(obj.Position) -- Mover para o saco de pancada
						humanoide:ChangeState(Enum.HumanoidStateType.Physics) -- Garantir que o humanoide está em modo físico para o movimento

						-- Caso haja algum RemoteEvent ou função para "bater", você pode usar assim:
						local remoteEvent = obj:FindFirstChild("HitEvent") -- Aqui depende do seu jogo, você pode ter que alterar
						if remoteEvent then
							remoteEvent:FireServer() -- A ação de bater no saco, conforme o sistema do jogo
						end
					end
				end
			end
