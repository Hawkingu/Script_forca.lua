...
-- Dumbbell Auto Farm
criarBotao("Auto Dumbbell", 50, function()
	while task.wait(0.5) do
		pcall(function()
			hrp.CFrame = game:GetService("Workspace").Dumbbell.CFrame
		end)
	end
end)

-- Kill Farm (não jogar inimigo longe)
criarBotao("Farm Kill", 90, function()
	while task.wait(0.5) do
		for _, v in pairs(game.Players:GetPlayers()) do
			if v ~= plr and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
...
local flySpeed = 80
local flying = false
criarBotao("Ativar Voo no Chão", 130, function()
...
criarBotao("Parar Voo", 170, function()
	flying = false
end)

-- Noclip
local noclip = false
criarBotao("Ativar Noclip", 210, function()
	noclip = true
